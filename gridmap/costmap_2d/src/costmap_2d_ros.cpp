    ...
    footprint_padding_(0.0)
{
  ...
  private_nh.param("rolling_window", rolling_window, false);    //是否滑动窗口（局部窗口）
  private_nh.param("track_unknown_space", track_unknown_space, false);  //是否允许未知区域存在（是否把未知当做障碍物）
  private_nh.param("always_send_full_costmap", always_send_full_costmap, false);  //是否允许发送整个costmap

  layered_costmap_ = new LayeredCostmap(global_frame_, rolling_window, track_unknown_space);  //建立一个costmap
  
  //如果没有costmap层的插件，加载旧插件
  ...
  if (private_nh.hasParam("plugins"))
  {
    ...
    for (int32_t i = 0; i < my_list.size(); ++i)
    {
      ...
      //一层层的把plugin添加到layered_costmap_中
  ...
  // 创建新线程
  stop_updates_ = false;  //停止更新
  initialized_ = true;    //是否初始化了
  stopped_ = false;       //是否停止

  // Create a time r to check if the robot is moving
  robot_stopped_ = false;   //机器人是否停止移动了
...
/**
* 根据输入的points，进行一次pad，然后set到layered_costmap中
*/
...
void Costmap2DROS::mapUpdateLoop(double frequency)
{
  ...
  while (nh.ok() && !map_update_thread_shutdown_)
  {
    ...
    struct timeval start, end;    //地图开始更新和地图结束更新的时间
    double start_t, end_t, t_diff;  //地图开始更新的地图结束更新的时间、时间差（更新地图所需要的时间）
...
void Costmap2DROS::updateMap()
{
  ...
  if (!stop_updates_)
  {
    ...
    if (getRobotPose (pose))
    {
      ...
      //调用 layered_costmap的updatemap来更新
